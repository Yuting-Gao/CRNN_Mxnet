...
class TextIter(mx.io.DataIter):
    def __init__(self, path , data_root, batch_size,
                 ...
                 get_image_function=None, read_content=None,data_shape=[32,800],buckets=[4,8,12,16,20,24,28,32,36,40]):
        super(TextIter, self).__init__()
        #self.masks=mask_generate(data_root)
        ...
        self.bucket_images,self.bucket_labels,self.data_plan=self.make_buckets(buckets,data_root,batch_size)
    ...
    def make_buckets(self,buckets,data_root,batch_size):
        ...
        for label_idx,img in enumerate(self.imagepaths):
            if not os.path.exists(data_root+'/'+img):
            	continue
            try:
            ...
            if ratio*4 > 25:
                ...
                labe_len = ratio*4
            bucket_labels[ratio-1].append(self.imagelabels[label_idx][0:labe_len])
    ...
    def next(self):
        if self.iter_next():
            ...
            data=self.get_image_function(self.bucket_images[buck_idx][img_idx*self.batch_size:img_idx*self.batch_size+self.batch_size],self.data_root)
            label=self.bucket_labels[buck_idx][img_idx*self.batch_size:img_idx*self.batch_size+self.batch_size]
